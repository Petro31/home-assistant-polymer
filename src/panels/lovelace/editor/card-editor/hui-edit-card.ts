...
export class HuiEditCard extends LitElement {
  ...
  private _cardYaml: string | undefined;

  private get cardYaml(): string | undefined {
    if (!this._cardYaml) {
      this._cardYaml = yaml.safeDump(this._cardConfig);
    }
    return this._cardYaml;
  }

  private set cardYaml(yml: string | undefined) {
    this._cardYaml = yml;
  }
